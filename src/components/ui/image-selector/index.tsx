...
export const ImageSelector = forwardRef<HTMLInputElement, ImageSelectorProps>((props, ref) => {
  const {
  ...
  const fileRemove = (event: { stopPropagation: () => void }, indexToRemove: number = 0) => {
  ...
  return (
    <div {...getRootProps()}>
      <div className="flex h-[170px] w-[170px] cursor-pointer items-center justify-center rounded border">
        {files.length === 0 && <CameraIcon height={80} width={80} className="text-primary/50" />}
        {files.map((file, index) => (
          ...
          >
            <Image
              ...
              src={file.preview}
              fill
              className="h-[150px] w-[150px] rounded-sm object-contain"
      ...
      {errorMessage && (
        <p className={cn('text-destructive text-sm font-medium')}>{String(errorMessage)}</p>
      )}
    </div>
  );
});
