...
const DOCUMENT_RESUME = ['FT', 'FR', 'OR'];
const DOCUMENT_PAYMENT = ['FT', 'FR', 'RE'];
const DOCUMENT_REQUIRE_TAX = ['FT', 'FR', 'OR'];
const DOCUMENT_RELATED = ['RE'];
...
export function InvoiceDetails({ invoice }: { invoice: InvoiceDetailsData }) {
  ...
  const showResume = DOCUMENT_RESUME.includes(invoice.type);
  const showPayment = DOCUMENT_PAYMENT.includes(invoice.type);
  const showTax = DOCUMENT_REQUIRE_TAX.includes(invoice.type);
  const showDocumentRelated = DOCUMENT_REQUIRE_TAX.includes(invoice.type);
  ...
  return (
    <div className="container mx-auto py-10">
        ...
        <Card>
          ...
          <CardContent>
            <dl className="grid grid-cols-2 gap-2">
              ...
              <dd>{invoice.exchange ? currencyFormatter(invoice.exchange || 0) : 'S/N'}</dd>
            </dl>
          </CardContent>
        </Card>
      ...
      {/* Resumo Financeiro */}
      {(showPayment || showResume) && (
        <div
          className={cn(
            'mt-6 grid gap-6 md:grid-cols-2',
            (showResume || showPayment) && 'md:grid-cols-1',
          )}
        >
          {showResume && (
            <Card>
      ...
      {(showDocumentRelated || showTax) && (
        <div
          className={cn(
            'mt-6 grid gap-6 md:grid-cols-2',
            (showDocumentRelated || showTax) && 'md:grid-cols-1',
          )}
        >
          {showDocumentRelated && (
            <Card>
