...
export async function POST(
  ...
  try {
    ...
    const receiptData = await prepareData({
      ...
      payments,
      documents: [{ documentId: invoice.id, paid: payments.reduce((total, item)=>total+item.amount, 0) }],
    });
    ...
    const paymentResponse =  prisma.invoicePayment.createMany({
      data: receiptData.paymentsData.map((payment) => ({
        ...payment, id: randomUUID(), invoiceId: params.id, observation: `${payment.observation || ''} ${receiptData.number}`.trim()
      })),
    })
