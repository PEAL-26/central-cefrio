...
export async function downloadBase64JVB(str: string, filename = 'PDF_File') {
  // Verifica se a string não está vazia e possui um formato válido
  if (!str || typeof str !== 'string') {
    throw new Error('A string fornecida não é um base64 válido para um arquivo PDF.');
  }
  ...
  downloadLink.download = `${createSlug(filename).toUpperCase()}.pdf`;
